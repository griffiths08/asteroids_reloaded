...
void main(void) 
{
    ...
    // subtle zoom in/out
    vec2 uv = 0.5 + (q-0.5)*(0.98 + 0.001*sin(0.9*time));
    ...
    col.r = texture2D(tex0,vec2(uv.x+0.002+(sin(time)*0.002),uv.y)).x;
    col.g = texture2D(tex0,vec2(uv.x+0.000,uv.y)).y;
    col.b = texture2D(tex0,vec2(uv.x-0.002-(sin(time)*0.002),uv.y)).z;
...
col = clamp(col*0.5+0.5*col*col*1.2,0.0,1.0);
   ...
   col *= 1.0-0.2*rand(vec2(time, tan(time)));
   // col *= 1.0-0.07*rand(vec2(time, tan(time)));

    //smoothen
    float comp = smoothstep( 0.2, 0.7, 0 );
    col = mix( col, oricol, clamp(-2.0+2.0*q.x+3.0*comp,0.0,1.0) );

    gl_FragColor = vec4(col,texture2D(tex0,vec2(q.x,q.y)).a);
}
